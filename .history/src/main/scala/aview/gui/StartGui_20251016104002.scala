...
object GUI extends SimpleSwingApplication {
        ...
        case ButtonClicked(_) =>
          SwingUtilities.invokeLater(new Runnable {
            def run(): Unit = {
              ...
              WebLog.log(s"GUI: set player count = $playerName") // <— NEU
            }
          })
      }
    }
          ...
          SwingUtilities.invokeLater(new Runnable {
            def run(): Unit = {
              ...
              if (controller.gameLogic != null && staticMatrix.isEmpty) {
                ...
                WebLog.log(s"GUI: set player count = $cardCount") // <— NEU
              }
            }
          })
      }
  ...
  class CardButton(val row: Int, val col: Int, val card: Card) extends Button {
    ...
    def revealCard(): Unit = {
      ...
      WebLog.log(card.id.toString)
      WebLog.log(s"GUI: open ${row+1} ${col+1} location board")
