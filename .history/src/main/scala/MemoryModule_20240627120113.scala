...
import model.FileIOComponent.FileIOJson._
...
class MemoryModule extends AbstractModule {
  ...
  override def configure(): Unit = {
    ...
    //bind(classOf[FileIOInterface[Card]]).to(classOf[FileIO])
    bind(new TypeLiteral[FileIOInterface[Card]]() {}).to(classOf[FileIO])
