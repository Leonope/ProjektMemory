...
class FileIO extends FileIOInterface[Card] {
  ...
  override def load: IMatrix[Card] = {
    val file = new File("matrix.json")
    if (!file.exists()) {
      // Erstelle eine neue Matrix, wenn die Datei nicht existiert
      println("matrix.json not found, creating a new matrix.")
      return new Matrix[Card](4, 4, new Card(0, FaceDownState))
    }
  ...
  def writes(card: Card): JsObject = Json.obj(
    "id" -> card.id,
