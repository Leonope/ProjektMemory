...
def getPlayerCount: Option[Int] = playerCount
  ...
  def startGameLogic(): Unit = {
    ...
    gameOutcome match {
      case Success((name, matrix)) =>
        ...
        println(matrix)
        loadedMatrix = matrix.asInstanceOf[Matrix[Card]]
  ...
  def redo(): Unit = {
    ...
    override def uiString: String = {
    // Nimm hier etwas Sinnvolles: Board als String, Spielstand etc.
    // Beispiele, je nachdem was du hast:
    // gameLogic.board.toString
    // gameLogic.render()
    // viewString()
    String.valueOf(gameLogic) // Platzhalter: bitte auf deine API mappen
  }

  override def handleWeb(cmd: String): Unit = {
    cmd.trim.toLowerCase.split("\\s+").toList match {
      case "restart" :: Nil    => restartGame()
      case "undo" :: Nil       => undo()          // falls vorhanden
      case "redo" :: Nil       => redo()          // falls vorhanden
      case "open" :: r :: c :: Nil =>
        // Beispiel: Karte umdrehen
        // open(r.toInt, c.toInt)  // an deine API anpassen
      case _ =>
        // optional: ignorieren oder eine Meldung setzen
    }
  }
  }
}
