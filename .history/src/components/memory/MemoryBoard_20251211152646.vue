  ...
  <div class="board">
    ...
    >
      <button
        class="memory-card"
        type="button"
        :class="card.state"   <!-- facedown / faceup / matched -->
        @click="$emit('flip', index)"
      >
        <div class="card-inner">
          <span class="symbol">
            <!-- Hier entscheidest du, was gezeigt wird -->
            <!-- card.symbol = deine Paar-Nummer (1,2,3,...) -->
            {{ card.state === 'facedown' ? '?' : card.symbol }}
            <!-- Wenn du lieber die ID willst:
...
.card-inner {
  ...
  transition: transform 0.35s ease, background 0.2s, color 0.2s, border-color 0.2s;
}
