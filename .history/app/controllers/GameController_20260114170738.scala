...
class GameController @Inject()(
  ...
  requireLogin: RequireLogin // ✅ injiziert
  ...
  // --- Neue Bootstrap-UI ---
  // ✅ Nur nach Login erreichbar:
  def indexBootstrap: Action[AnyContent] = requireLogin { implicit request =>
    Ok(
  ...
  // ✅ Nur nach Login erreichbar:
  def newGameBootstrap: Action[AnyContent] = requireLogin { implicit request =>
  ...
  // ✅ Nur nach Login erreichbar:
  def newGameBootstrapJson: Action[JsValue] = requireLogin(parse.json) { implicit request =>
