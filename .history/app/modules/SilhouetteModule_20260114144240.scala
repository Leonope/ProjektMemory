...
import play.silhouette.impl.providers.{OAuth2Settings, SocialProviderRegistry, SocialStateHandler}
import play.silhouette.impl.providers.oauth2.GitHubProvider
import play.silhouette.impl.providers.state.{CsrfStateItemHandler, DefaultSocialStateHandler}
...
class SilhouetteModule extends AbstractModule {
  ...
  override def configure(): Unit = {
    ...
    // Silhouette sauber über Guice binden (Provider + Default-Action-Module von Silhouette)
    // Siehe Silhouette Docs: bind[Silhouette[Env]].to[SilhouetteProvider[Env]]
  ...
  @Provides @Singleton
  def provideSocialStateHandler(csrfStateItemHandler: CsrfStateItemHandler): SocialStateHandler =
    new DefaultSocialStateHandler(csrfStateItemHandler)
  ...
  def provideGitHubProvider(
    ...
    // GitHub OAuth2 Endpoints (SpecifiedSettings gibt’s in deiner Version nicht)
    val settings = OAuth2Settings(
      authorizationURL = "https://github.com/login/oauth/authorize",
      accessTokenURL   = "https://github.com/login/oauth/access_token",
      redirectURL      = Some(callbackUrl),
