(function () {
            ...
            <div class="card-outer shadow-sm">
              <div class="card-inner">
                <div class="card-face card-back d-flex align-items-center justify-content-center">
                  <span class="card-back-image"></span>
                </div>
  ...
  // ====== Bilder setzen (Front + Back) ======
  function setCardFaceImage(faceEl, symbol) {
    if (!faceEl) return;
    const src = `/assets/images/cards/${encodeURIComponent(symbol)}.jpeg`;
    faceEl.innerHTML = `<img src="${src}" alt="Card ${String(symbol)}">`;
  }

  function setCardBackImage(backEl) {
    if (!backEl) return;
    backEl.innerHTML = `<img src="/assets/images/cards/back.jpeg" alt="Card Back">`;
  }
  ...
  function initBoard() {
    ...
    cards.forEach((card, idx) => {
      ...
      const front = card.querySelector('.symbol');
      const back  = card.querySelector('.card-back-image');

      if (front) setCardFaceImage(front, symbol);
  ...
  // Bei jedem Öffnen des Collapses: Highscores neu laden
  const el = document.getElementById('highscoreCollapse');
  if (!el) return;
