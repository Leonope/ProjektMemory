(function () {
  ...
  // ====== NEU: Bild in die Kartenfront setzen (Variante A) ======
  function setCardFaceImage(faceEl, symbol) {
    if (!faceEl) return;
    // erwartet z.B. public/images/cards/1.png  -> /assets/images/cards/1.png
  ...
  if (!faceEl) return;
  const src = `/assets/images/cards/${encodeURIComponent(symbol)}.jpeg`;
  faceEl.innerHTML = `<img src="${src}" alt="Card ${symbol}">`;
}
